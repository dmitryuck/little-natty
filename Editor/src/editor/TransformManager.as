package editor {
	...
	{		
		...
		public const TRANSFORM_MODE_ROTATION:String = "TRANSFORM_MODE_ROTATION";
		public const TRANSFORM_MODE_SCALE:String = "TRANSFORM_MODE_SCALE";
		
		public const DEFAULT_MIN_SCALE:Number = 0.1;
		public const DEFAULT_MAX_SCALE:Number = 10;
		
		protected var me:TransformManager;
		...
		public function TransformManager(showInterestingStuff:Boolean = false):void 
		{			
			me = this;
			iMatrix = new Matrix();
			iMatrix.identity();
			...
			me.addEventListener(MouseEvent.ROLL_OVER, onRollOver);
			me.addEventListener(MouseEvent.ROLL_OUT, onRollOut);
			
			_boundingRect.addEventListener(MouseEvent.MOUSE_DOWN, onDispObjMouseDown);	
			_boundingRect.addEventListener(MouseEvent.MIDDLE_CLICK, onDispObjMiddleClick);		
			
			mustReactivate = true;
		}		
		
		// Удалить выделенный обьект нажатием колеса мыши
		private function onDispObjMiddleClick(e:MouseEvent):void 
		{ 
			if (dispObj && e.target == _boundingRect)
			{
				Main.inspector.removeObject(dispObj);
			}
		}
		...
		{			
			...
			function _onSpriteFocus(e:MouseEvent):void 
			{				
				if (me.enabled)
				{					
					//activateSprite(e.target as DisplayObject, config);					
					
					activateSprite(e.currentTarget as DisplayObject, config);
					onDispObjMouseDown(e);					
				}
			}
		}		
		
		// Вычеркнуть спрайт со списка регистрации
		public function unRegisterSprite(sourceSprite:DisplayObject):void
		{
		...
		// Деактивировать спрайт
		public function deactivateSprite():void
		{
			disableHandlers();
			hideHandlers();
		...
		{			
			if (Game.scene.getActiveLayer() != sourceSprite.parent) return;
			
			if (dispObj == sourceSprite) return;
			...
			{				
				...
				dispatchEvent(new TransformEvent(TransformEvent.ON_FOCUS, dispObj));
				...
				dispatchEvent(new TransformEvent(TransformEvent.ON_TRANSFORM, dispObj, _storedX, _storedY, _storedRotation, _storedScale));
			}
		}		
		
		// Move
		protected function onDispObjMouseDown(e:MouseEvent):void
		{
		...
		public function onHandlerDown(e:MouseEvent):void
		{			
			mode = 1;
			mIsDown = true;
		...
		protected function onHandlerMouseDown(e:MouseEvent):void
		{
			transformMode = TRANSFORM_MODE_ROTATION;
			onHandlerDown(e);
		}
		
		protected function onHandlerRightMouseDown(e:MouseEvent):void
		{
			transformMode = TRANSFORM_MODE_SCALE;
			onHandlerDown(e);
		}
		...
		protected function onStageRightMouseUp(e:MouseEvent):void
		{
			mIsDown = false;
			clear();
		}
		...
		protected function buildHandles():void
		{
			...
			_tlHandler.addEventListener(MouseEvent.MOUSE_DOWN, onHandlerMouseDown);
			_trHandler.addEventListener(MouseEvent.MOUSE_DOWN, onHandlerMouseDown);
			_brHandler.addEventListener(MouseEvent.MOUSE_DOWN, onHandlerMouseDown);
			_blHandler.addEventListener(MouseEvent.MOUSE_DOWN, onHandlerMouseDown);
			
			_tlHandler.addEventListener(MouseEvent.RIGHT_MOUSE_DOWN , onHandlerRightMouseDown);
			_trHandler.addEventListener(MouseEvent.RIGHT_MOUSE_DOWN , onHandlerRightMouseDown);
			_brHandler.addEventListener(MouseEvent.RIGHT_MOUSE_DOWN , onHandlerRightMouseDown);
			_blHandler.addEventListener(MouseEvent.RIGHT_MOUSE_DOWN , onHandlerRightMouseDown);
		}
		...
		protected function doTransform(angle:Number, scale:Number):void
		{
			...
			if (transformMode == TRANSFORM_MODE_ROTATION) _storedRotation = angle + angleOffset;
			else if (transformMode == TRANSFORM_MODE_SCALE) _storedScale = scale * scaleOffset;
